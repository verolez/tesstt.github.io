...
$dashboardaccount = $link->query("SELECT accounts.account_id,accounts.account_name,productrelease.account,COUNT(productrelease.id) FROM `accounts` LEFT JOIN `productrelease` ON accounts.account_name = productrelease.account GROUP BY accounts.account_name") or die($link->error);
...
for ($color=0; $color < 50; $color++) { 
	...
	$checkvalue = $color % 6;
	if ($checkvalue == 0) {
		$buttoncolor[$color] = "btn-secondary";
	}elseif ($checkvalue == 1) {
		$buttoncolor[$color] = "btn-success";
	}elseif ($checkvalue == 2) {
		$buttoncolor[$color] = "btn-danger";
	}elseif ($checkvalue == 3) {
		$buttoncolor[$color] = "btn-warning";
	}elseif ($checkvalue == 4) {
		$buttoncolor[$color] = "btn-info";
	}elseif ($checkvalue == 5) {
		$buttoncolor[$color] = "btn-dark";
	}
}
...
$productdelete = $link->query("SELECT id,product_name FROM productrelease  ORDER BY id DESC LIMIT 10") or die($link->error);
	...
	for ($unknown = 0;$addaccountrow = $addaccount->fetch_assoc();$unknown++) {
		$assignaccount[$unknown] = $addaccountrow['account_name'];
	}
