...
if (isset($_GET['status'])) {
	...
	if ($status == "All") {
		$productreleaseview = $link->query("SELECT * FROM productrelease") or die($link->error);
		$productreleasetable = $link->query("SELECT id,control_num,assign_to,lot_num,account,status,date_added,date_modified FROM productrelease") or die($link->error);
		$productreleaseedit = $link->query("SELECT * FROM productrelease") or die($link->error);
$link->query("SELECT id,product_name FROM productrelease") or die($link->error);
		$productreleasedelete = $link->query("SELECT id,product_name,status FROM productrelease") or die($link->error);
	}else {
		$productreleaseview = $link->query("SELECT * FROM productrelease WHERE status = '$status'") or die($link->error);
		$productreleasetable = $link->query("SELECT id,control_num,assign_to,lot_num,account,status,date_added,date_modified FROM productrelease WHERE status = '$status'") or die($link->error);
		$productreleaseedit = $link->query("SELECT * FROM productrelease WHERE status = '$status'") or die($link->error);
		$productreleasedelete = $link->query("SELECT id,product_name,status FROM productrelease WHERE status = '$status'") or die($link->error);
	}
}

	$allreleasestatusdisplayalltemp = $link->query("SELECT status,COUNT(id) FROM `productrelease`") or die($link->error);
	$allreleasestatusdisplayall = $allreleasestatusdisplayalltemp->fetch_assoc();
...
if (isset($_POST['add'])) {
	...
	if (mysqli_query($link, $sql)) {
		echo '<script> window.location.href="allrelease.php?status=All"; </script>';	
	} else {
