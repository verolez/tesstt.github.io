...
if (isset($_GET['account']) && isset($_GET['status'])) {
	$account = $_GET['account'];
	$status = $_GET['status'];

	if ($status == "All") {
		$productreleaseview = $link->query("SELECT * FROM productrelease WHERE account = '$account'") or die($link->error);
		$productreleasetable = $link->query("SELECT id,control_num,assign_to,lot_num,account,status,date_added,date_modified FROM productrelease WHERE account = '$account'") or die($link->error);
		$productreleaseedit = $link->query("SELECT * FROM productrelease WHERE account = '$account'") or die($link->error);
$link->query("SELECT id,product_name FROM productrelease WHERE account = '$account'") or die($link->error);
		$productreleasedelete = $link->query("SELECT id,product_name,status FROM productrelease WHERE account = '$account'") or die($link->error);
	}else {
		$productreleaseview = $link->query("SELECT * FROM productrelease WHERE account = '$account' && status = '$status'") or die($link->error);
		$productreleasetable = $link->query("SELECT id,control_num,assign_to,lot_num,account,status,date_added,date_modified FROM productrelease WHERE account = '$account' && status = '$status'") or die($link->error);
		$productreleaseedit = $link->query("SELECT * FROM productrelease WHERE account = '$account' && status = '$status'") or die($link->error);
		$productreleasedelete = $link->query("SELECT id,product_name,status FROM productrelease WHERE account = '$account' && status = '$status'") or die($link->error);
	}

	$statusdisplayalltemp = $link->query("SELECT status,COUNT(id) FROM `productrelease` WHERE account = '$account'") or die($link->error);
	$statusdisplayall = $statusdisplayalltemp->fetch_assoc();

	$statusdisplayongoingtemp = $link->query("SELECT status,COUNT(id) FROM `productrelease` WHERE account = '$account' && status = 'Ongoing'") or die($link->error);
	$statusdisplayongoing = $statusdisplayongoingtemp->fetch_assoc();

	$statusdisplayforreviewtemp = $link->query("SELECT status,COUNT(id) FROM `productrelease` WHERE account = '$account' && status = 'For Review'") or die($link->error);
	$statusdisplayforreview = $statusdisplayforreviewtemp->fetch_assoc();

	$statusdisplayapprovedtemp = $link->query("SELECT status,COUNT(id) FROM `productrelease` WHERE account = '$account' && status = 'Approved'") or die($link->error);
...
if (isset($_POST['add'])) {
	...
	if (mysqli_query($link, $sql)) {
		...
		echo '<script> window.location.href="productrelease.php?account='.$account.'&status=All"; </script>';
	} else {
...
if (isset($_GET['approved']) && isset($_GET['account']) && isset($_GET['status'])) {
